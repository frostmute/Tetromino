...
if (!content.includes('import { downloadBinaryFile }')) {
	content = 'import { downloadBinaryFile } from "./downloadUtils";\n' + content;
}

const oldFunc = `	async downloadBinary(url: string): Promise<ArrayBuffer> {
		let attempts = 0;
...
const newFunc = `	async downloadBinary(url: string): Promise<ArrayBuffer> {
