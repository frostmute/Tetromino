...
import type { SyncConflictActions } from "./modals";
...
export default class TetrominoPlugin extends Plugin {
  ...
  async onload(): Promise<void> {
    ...
    this.engine = new SyncEngine(this.app, this.api, this.settings, (progress) =>
  ...
  async runSync(dryRun = false): Promise<void> {
    ...
    if (this.isMigrating) {
      new Notice(
        "Attachment migration is running. Try again after it finishes.",
      );
      return;
    }
  ...
  async runChannelSync(slug: string, dryRun = false): Promise<void> {
    ...
    if (!mapping) {
      new Notice(`Channel "${slug}" not found in settings or is disabled.`);
      return;
    }
  ...
  private updateStatusBar(state: "idle" | "syncing", detail = ""): void {
    ...
    if (state === "syncing") {
      (this.statusBarItem as HTMLElement & { setText: (text: string) => void }).setText(
        detail || "Importing from Are.na...",
      );
    } else {
      (this.statusBarItem as HTMLElement & { setText: (text: string) => void }).setText("");
    }
  ...
  private notifySyncResult(result: SyncResult): void {
    ...
    const prefix = result.dryRun ? "Tetromino preview" : "Tetromino";
  ...
  private showSummaryModal(result: SyncResult, label: string): void {
    ...
    const actions: SyncConflictActions | undefined = result.dryRun
      ? undefined
      : {
  ...
  private normalizeMappings(): boolean {
    let changed = false;
    this.settings.channelMappings = this.settings.channelMappings.map((mapping) => {
  ...
  async importMyChannelsMappings(): Promise<{
    ...
    for (const remote of remoteChannels) {
      ...
      if (existing) {
        ...
        if (
          beforeId !== existing.channelId ||
          beforeTitle !== existing.channelTitle
        ) {
          updated++;
        }
        continue;
  ...
  private async restoreFromBackupData(data: unknown): Promise<void> {
    ...
    this.settings.channelMappings = (backup.channelMappings as ChannelMapping[]).map(
      (mapping) => ({ ...mapping }),
    );
  ...
  async runMigration(planOverride?: MigrationPlan): Promise<void> {
    if (this.isMigrating) {
  ...
  private async writeMigrationReport(report: MigrationReport): Promise<void> {
    const folder = normalizePath("Are.na");
