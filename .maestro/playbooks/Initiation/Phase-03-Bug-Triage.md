...
- [ ] Search existing codebase for relevant bug locations: For a confirmed bug:
...
- [ ] Create a targeted test case for the bug: Before fixing:
...
- [ ] Implement the bug fix with confidence: With a failing test in place:
...
- [ ] Update CHANGELOG and documentation for bug fixes: After a successful fix:
...
- [ ] Verify the fix in a test vault (when practical): If the bug is user-facing:
...
- [ ] Close the issue with context and reference the fix: Once the fix is merged:
